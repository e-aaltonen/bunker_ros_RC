...
namespace westonrobot
{
  ...
  void BunkerROSMessenger::SetupSubscription()
  {
    ...
    rc_publisher_ =
        nh_->advertise<bunker_msgs::BunkerRCState>("/bunker_rc_status", 10); // EA
    ...
    if(use_scan_)
      scan_subscriber_ = nh_->subscribe<sensor_msgs::LaserScan>(
          "/scan", 100, &BunkerROSMessenger::ScanCallback, this);
  }
  ...
  void BunkerROSMessenger::ScanCallback(const sensor_msgs::LaserScan::ConstPtr &msg)
  {
      sensor_msgs::LaserScan scan = *msg;
      float temp = 10.0;
      int index = 0;
      for (int i=786; i<1310; i++)  //0~180
      {
      ...
      if(temp <= 1.0)
      {
          ...
          cout << "scan stop !!!" << endl;
          bunker_->scan_stop = true;
      }
      else
      {
          cout << "go !!!" << endl;
          bunker_->scan_stop = false;
      }
  }
  ...
  void BunkerROSMessenger::PublishStateToROS()
  {
    ...
    rc_msg.right_stick_left_right = state.right_stick_left_right;   
    rc_msg.right_stick_up_down = state.right_stick_up_down;         
    rc_msg.left_stick_left_right = state.left_stick_left_right;     
    rc_msg.left_stick_up_down = state.left_stick_up_down;           
